...
new_termo   : {self.begin_termo()} termo {self._last_termo.var_type=self._last_termo_type}
            ;
termo       : (ID {self.acces_var(self._input.LT(-1).text)} {self.append_to_termo(self._symbol_table[self._input.LT(-1).text])}
            | NUM {self._last_termo_type=VarTypes.NUM} {self.append_to_termo(Num(self._input.LT(-1).text))}
            | TEXTO {self._last_termo_type=VarTypes.TEXT} {self.append_to_termo(Text(self._input.LT(-1).text))}
            | BOOL {self._last_termo_type=VarTypes.BOOL} {self.append_to_termo(Boolean(self._input.LT(-1).text))}
